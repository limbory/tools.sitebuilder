...
// require('../../common/javascript/assets/jquery');
var $ = require('jquery-legacy/node_modules/jquery');

window.$ = window.jQuery = $;
require('../../common/javascript/assets/jquery.dotdotdot');
...
$(function () {
  ...
  (function () {
    ...
    if (!/(search|tags)/.test(pageLocation)) { 
      return false;
    }
    ...
    if (/search/.test(pageLocation)) {
      ...
      $.each(window.postMsg, function (i, d) {
        if (judgeString(i)) {
          var $inner = $('<li><span>'+ d.date +'</span><a href="'+ d.url +'" title="'+ i +'" >'+ hundleString(i) +'</a></li>');
          if (d.link) {
            $inner.find('a').attr({'href': d.link, 'target': '_blank'}).append(' <img class="link-ico" src="'+window.BASE_URL+'/img/link-sm.png">');
          }
    ...
    if (searchResultAreaInner.find('ul').children().length > 0) {
  ...
  (function () {
    ...
    if ($comment.length >= 1 && $('body').is('.at-index-page') ) {
      ...
      getCountData(window.postSiteID, 0);
    }
  })();
  ...
  /* dotdotdot插件 */
  (function () {
    var $initObj = '.header-wrapper .posttitle .pt_2' + ',' + '.rp-widget li h3 .pt_2';
    $($initObj).dotdotdot({
      watch: 'window',
      wrap: 'letter'
    });
...
[1,2,3,].forEach((d) => window.alert(d));
window.alert(!!Object.defineProperty);